...
public class Account {
	 ...
	 *
	 * 동기화처리 결국 객체 단위로 동기화 처리. account 객체를 임계 영역(한번에 한쓰레드만 들어갈 수 있는 영역)으로 설정하게된다. 1.
	 * synchronized 메소드 만들기 //public synchronized void withdraw(int money) { 2.
	 * synchronized block 사용하기 (추천)
	 * 
	 */
	public void withdraw(int money) {
		String threadName = Thread.currentThread().getName();

		synchronized (this) { //this 에 객체가 들어감?
